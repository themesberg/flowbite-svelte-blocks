...
<Section class="mx-auto max-w-8xl">
  ...
  {#if !expanded && components.length > INIT_COUNT}
    <div class="w-full flex justify-center mb-4">
      <Button size="md" class="whitespace-nowrap hover:text-primary-600 focus:text-primary-600" color="alternative" onclick={() => (expanded = true)}>View all components</Button>
    </div>
  {/if}
</Section>